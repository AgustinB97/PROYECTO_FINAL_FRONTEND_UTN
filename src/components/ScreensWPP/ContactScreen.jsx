...
const ContactScreen = () => {
    ...
    useEffect(() => {
        ...
        const loadChats = async () => {
            const res = await getUserChats(user._id);
            if (res.ok) {
                setChats(res.chats);
            } else {
                console.error("[ContactScreen] Error al cargar chats:", res.message);
            }
        };
    ...
    useEffect(() => {
        ...
        const handleReceiveMessage = (msg) => {
            ...
            setChats(prev => {
                ...
                const updated = prev.map(c => c._id === chatId ? { ...c, last_message: msg } : c);
    ...
    useEffect(() => {
        ...
        socket.on("chats_updated", handleChatsUpdated);

        return () => {
            socket.off("chats_updated", handleChatsUpdated);
        };
    }, [socket, user, setChats]);

    const filteredChats = chats.filter(chat => {
        if (chat.isGroup) return chat.name?.toLowerCase().includes(searchText.toLowerCase());
    ...
    return (
        <div className="contact-screen">
            ...
            <form className="search-contact">
                <input
                    type="text"
                    placeholder="Buscar chat o crear uno nuevo"
                    value={searchText}
            ...
            <ul className="contacts-list">
                {filteredChats.map(chat => {
                    ...
                    if (chat.isGroup) {
                        name = chat.name;
                        avatar = chat.avatar || "/default-group.png";
                    } else {
                        other = chat.members.find(m => String(m._id) !== String(user._id));
                    ...
                    return (
                        <li
                            ...
                            style={{
                                backgroundColor: selectedChat?._id === chat._id ? "#ebe6e6ff" : undefined
                            }}
                        >
                            <div className="contact-preview__avatar-container">
                                <img className="contact-preview__avatar" src={avatar} alt="avatar" />
                            </div>

                            <div className="contact-preview__text">
                                <div className="contact-preview__name-hour">
                                    <h2 className="contact-preview__name">{name}</h2>
                                ...
                                <span className="contact-preview__last-message">
                                    {last ? last.content : "No hay mensajes"}
                                </span>
                            </div>
                        </li>
                    );
